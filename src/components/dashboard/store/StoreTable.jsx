...
import { useDeleteStore, useGETStore } from "@/hooks/store.hooks";
...
const DeleteAction = ({ id }) => {
  const { mutateAsync, isLoading: isDeletting } = useDeleteStore(id);
    ...
    <div className="px-2 lg:px-[35px] space-y-5">
      <div className="bg-[#F3F3F6] p-4 lg:p-10 rounded-[20px]">
        <div className="flex justify-between">
          <div className="text-[22px] lg:text-[36px] font-bold">Store</div>
                ...
                {data &&
                  data.slice(offset, limit).map((info, idx) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">{info.outlet}</td>
                      <td className="px-6 py-4">{info.number}</td>
                      <td className="px-6 py-4">{info.email}</td>
                      <td className="px-6 py-4 text-bbc-dash-3  min-w-[200px] max-w-[310px] capitalize">
                        {info.address}
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex gap-2 justify-end">
                          <StoreForm instance={info} />
